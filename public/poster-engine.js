...
/* fields where the user enters multiple short items (rooms, kitchen, garden...) instead of one free-text line */
const MULTI_FIELDS = new Set(["amenities","work"]);
...
const SHAPES=[
 ...
 {id:"curve",name:"منحنى ناعم"},
 {id:"circle",name:"دائرة أنيقة"},
...
/* returns an array of [x,y] fraction points (0..1) describing the image-area clip polygon for a shape,
   or null for shapes handled without a point-polygon (e.g. "frame", "circle"). Shared by DOM (clip-path) and canvas (ctx path).
   Kept gentle/subtle on purpose: the cut is a design accent near one edge only, never eating deep into the photo. */
function shapeClipPoints(shape){
 if(shape==="diagonal") return [[0,0],[1,0],[1,0.90],[0,1]];
 if(shape==="curve"){
  const pts=[[0,0],[1,0]];
  const baseY=0.91, amp=0.05, steps=10;
  for(let i=steps;i>=0;i--){
   const x=i/steps;
   const y=baseY+amp*Math.sin(x*Math.PI);
   pts.push([x,y]);
  }
  return pts;
 }
 return null; // straight / frame / circle
}
...
async function renderExportCanvas(state){
 ...
 ctx.save();
 if(shape==="circle"){
  const r = Math.min(imgW,imgH)/2 * 0.98;
  ctx.beginPath();ctx.arc(imgX0+imgW/2, imgY0+imgH/2, r, 0, Math.PI*2);ctx.closePath();ctx.clip();
 } else if(clipPts){
  ctx.beginPath();
 ...
 try{
  ...
  ctx.restore();
 }catch(e){ /* logo not reachable, skip silently */ }

 if(shape!=="frame" && shape!=="circle" && shape!=="diagonal" && shape!=="curve"){
...
function drawDetails(ctx, entries, style, T, startY, availH, padX, W, RTL){
 ...
 function measureAndDraw(commit){
  ...
  if(style==="badges"){
   const cols=Math.min(entries.length,4)||1;
   const colW=(W-padX*2)/cols, rowH=118*scale, circleR=30*scale;
   entries.forEach((e,i)=>{
    const col=i%cols, row=Math.floor(i/cols);
    const cx = RTL ? (W-padX-colW*col-colW/2) : (padX+colW*col+colW/2);
    const cy=yy+row*(rowH+6*scale);
    if(commit){
     ctx.font="800 "+(12.5*scale*2)+"px Arial";ctx.fillStyle=T.title;ctx.textAlign="center";
     ctx.fillText(truncate(ctx,e.value,colW-16),cx,cy+16*scale);
     ctx.beginPath();ctx.arc(cx,cy+16*scale+circleR+8*scale,circleR,0,Math.PI*2);ctx.fillStyle=T.accent;ctx.fill();
     ctx.font=(22*scale)+"px Arial";ctx.fillStyle=T.accentText;ctx.textAlign="center";ctx.textBaseline="middle";
     ctx.fillText(e.icon,cx,cy+16*scale+circleR+8*scale+2);
     ctx.textBaseline="alphabetic";
     ctx.font="700 "+(10*scale*2)+"px Arial";ctx.fillStyle=T.sub;
     ctx.fillText(truncate(ctx,e.label,colW-10),cx,cy+16*scale+circleR*2+30*scale);
    }
   });
   const rows=Math.ceil(entries.length/cols);
   return rows*(rowH+6*scale);
  }
  return 0;
 }
}

window.PosterEngine={configs,ICONS,KEYWORD_ICONS,iconFor,splitItems,MULTI_FIELDS,THEMES,THEME_COLORS,IMGLAYOUTS,DETAILSTYLES,SHAPES,shapeClipPoints,imageLayoutRects,fieldEntries,renderExportCanvas,EN_CATEGORY_LABELS,EN_FIELD_LABELS};
})();
